...
char **recips;
{
 ...
 if (qqtalk_open(&qqt,1) == -1) temp_fork();
 mailforward_qp = qqtalk_qp(&qqt);
 qqtalk_put(&qqt,dtline.s,dtline.len);
 do
  {
   if (getline2(&ss,&messline,&match,'\n') != 0) { qqtalk_fail(&qqt); break; }
   qqtalk_put(&qqt,messline.s,messline.len);
  }
 while (match);
 qqtalk_from(&qqt,ueo.s);
...
void checkhome()
{
 ...
 if (st.st_mode & ALIAS_PATERNALISM) temp_homewritable();
 if (st.st_mode & 01000)
   ...
   if (fd == -1)
   ...
   else
    {
     ...
     if ((st.st_mode & S_IFMT) == S_IFREG)
      {
       if (st.st_mode & ALIAS_PATERNALISM) temp_qmwritable();
...
char **argv;
{
 ...
 if (!cmds.len)
  {
   if (!stralloc_copys(&cmds,ALIAS_EMPTY)) temp_nomem();
   flagforwardonly = 0;
  }
 if (cmds.s[cmds.len - 1] != '\n')
